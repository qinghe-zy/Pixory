...
| --- | --- | --- | --- |
| AI 陪伴聊天 | 已实现，重点方向 | 长聊天、角色扮演、记忆、资料上下文、分支和流式回复；DeepSeek V4 Prompt Ledger 前缀回放、视觉模型图片门禁，以及开发者模式下“设置 → 性能与诊断”的标准/深度诊断包入口已接入 | `AiHomeScreen`, `AiChatScreen`, `src/ai/`, `SettingsScreen`, `DiagnosticsSettingsScreen` |
