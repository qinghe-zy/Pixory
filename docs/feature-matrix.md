...
| 生成崩溃恢复 | V55 持久恢复 | 每次主聊天生成在 Provider 请求前持久化 job，首个 delta 转为 streaming，并按现有合批节奏同步部分正文/思考与 content-free 事件；完成、失败、超时和停止时消息与 job 同事务结算。进程重启后按空间单航班协调：无部分正文时对同一 assistant 占位最多自动重试一次，有部分正文时最多续写一次且不新增思考并做重叠去重；保存原人设/模型/分支快照但重新读取 SecureStore 密钥。普通空间初始化后恢复，Personal 仅成功解锁后恢复；跨空间迁移保留 job/event 且清除 lease | `src/ai/generation/`, `aiChatService`, `aiGenerationManager`, `App.tsx`, `ai_generation_jobs`, `ai_generation_events` |
