...
| 陪伴卡片版本与删除 | 日记、梦境卡片按稳定产物组锚定在原聊天位置，重生成仅追加新版本；卡片下方可切换 `1/2`，长按复用消息气泡的触点锚定菜单以重新生成或仅从当前聊天移除。聊天移除写入线程级隐藏状态，不会删除内心独白或历史消息；只有当前梦境版本可进入后续上下文。内心独白展示日记/梦境全部版本，长按进入当前标签的多选模式，统一确认后直接永久删除所选版本，不进入回收站 | `AiChatScreen`, `DiaryChatCard`, `DreamChatCard`, `AiAnchoredContextMenu`, `CompanionInnerLifeScreen`, `companionArtifactChatStateRepository` |
