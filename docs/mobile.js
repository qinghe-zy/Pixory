...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Ensure the single video plays and resumes if stalled
  const mainVid = document.getElementById('hero-vid-0');
  if (mainVid) {
    if (mainVid.classList.contains('active')) {
      mainVid.play().catch(() => {});
    }
    mainVid.addEventListener('canplay', () => {
      if (mainVid.classList.contains('active') && mainVid.paused) {
        mainVid.play().catch(() => {});
      }
    });
  }

  // Mobile browser autoplay policy workaround: play on touch interaction
  const tryPlayVideo = () => {
  ...
  document.addEventListener('touchstart', tryPlayVideo, { passive: true });
  document.addEventListener('click', tryPlayVideo, { passive: true });
  ...
  function showContent(nextContentIndex) {
    ...
    const videoSrcs = [
      "assets/videos/mobile_hf_20260702_081127_0992a171-d3c6-4978-8213-0ec5df8b6d63.mp4",
      "assets/videos/mobile_hf_20260702_092026_dd05b805-ea0f-40b2-8c52-332b88502592.mp4",
