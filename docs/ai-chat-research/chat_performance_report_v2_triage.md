...
| P0-3 | `AiMessageContent` memo + 正则缓存 | `部分成立` | 组件未 `React.memo`，`shouldRenderWholeRichHtml(content)` 每次渲染执行；但非流式 Markdown 已经用 `useMemo`，且存在 120 项 LRU 风格解析缓存；流式消息走轻量纯文本分支。 | 先补渲染计数/大消息基准；只缓存 rich-HTML 判定并评估组件 comparator，保留内部反馈状态和 `trailingInline` 正确性。 |
...
| P0-6 | 优化 `estimatePromptTokens` | `已验证`、收益待基准 | 当前 `value.match(CJK_CHAR_PATTERN)` 分配匹配数组，二分裁剪又对多个前缀重复扫描。 | 用无数组分配的单次字符扫描保持估算语义；为裁剪增加可复用前缀计数或等价低分配算法，并用中英日韩混合 corpus 验证结果完全一致。 |

## P1 核对

| # | 报告项 | 标记 | 当前证据 | 决策 |
|---|---|---|---|---|
| P1-7 | 嵌入生成并发批处理 | `已验证` | `generateMissingEmbeddingsForDocument()` 对每个 chunk 串行 `await adapter.embedText()`，失败只计数；完成后再统一写库。 | 采用 provider-aware 有界并发，不使用无上限 `Promise.all`。默认并发上限 3，保持结果与 chunk id 的确定映射并维持现有失败计数语义；重试/退避仍属于 P2，不混入本项。实际倍数由基准报告。 |
| P1-8 | KaTeX 编译缓存 | `已验证` | `AiMathBlock` 在 render body 执行 `katex.renderToString()` 和 HTML 拼接；WebView 高度回传会触发 `setHeight()` 后重渲染。 | 将“编译成功/失败 + HTML”做成以 `math` 为键的 memoized 结果；高度更新不得重复编译。 |
| P1-9 | knowledge repository 批量删除/写入 | `已验证` | `deleteDocument()` 对每个 chunk 连续执行两次删除；`replaceEmbeddings()` 对每个 embedding 执行 delete + insert。 | 在事务中使用子查询批量删除；写入采用受 SQLite 参数上限约束的分批 upsert。保留 Personal/normal space 的数据库隔离。 |
...
| P1-12 | 所有 JS 动画开启 Native Driver | `约束冲突`、`部分成立` | `resizeHandleOpacity` 可独立原生化；`petPan` 同时被 PanResponder、`setValue`、`addListener` 和定时动画共享，直接把某次 timing 改成 native driver 会混用 JS/native ownership。 | 拆分为安全的 opacity 小改与完整 pet gesture 状态迁移；禁止批量搜索替换 `useNativeDriver: false`。 |
...
1. P1-7 embedding provider-aware 有界并发；重试/退避留在 P2。
2. P0-1 detached tail 的局部订阅、稳定回调和 wrapper memo；是否引入 `useSyncExternalStore` 由 Wave 0 数据决定。
3. P1-12 中独立、安全的 opacity 动画调整。
