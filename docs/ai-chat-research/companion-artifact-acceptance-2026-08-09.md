...
| 日记手动和自动触发共用真实链路 | 手动确认、夜间自动触发和 Android wake 最终都进入 `prepareAndScheduleDiaryJob`，再由独立运行时执行；聊天页卸载不取消任务 | `diarySchedulerService.ts`、`diaryGenerationManager.ts`、`diaryRuntimeCoordinator.ts` | `role-diary-scheduler-policy.test.cjs`、`role-diary-visibility-and-chat-entry-policy.test.cjs` |
...
| 梦境每天最多两次 | 限制对象是同一 physical space 内同一角色的自动成功梦境；北京时间每天最多 2 次，跨线程合并，normal/personal 物理隔离 | `dreamRepository.ts`、`dreamPolicy.ts` | `companion-dream-repository-integration.test.cjs`、`companion-dream-policy-unit.test.cjs` |
...
| 跨日失败可直接重试 | 重试不要求再产生一个聊天轮；配额预留会在同一事务内滚动到新的北京时间日期，并原子更新任务预留和角色计数，避免双击/并发重复占额 | `dreamRepository.reserveDreamQuota` | `companion-dream-repository-integration.test.cjs` |
| 梦境失败重试有响应 | 可重试失败复用原 seed、roll 和冻结来源；来源变化时建立按当前消息冻结的手动替代任务；频率阻断、状态已变化和请求异常均在聊天页显示错误并重新加载卡片 | `dreamWorker.ts`、`dreamService.ts`、`AiChatScreen.tsx` | `companion-dream-recovery-unit.test.cjs` |
...
| 梦境生成不是空功能 | 候选门禁、结构化分类、持久 roll、独立生成 job、严格 JSON 解析、最多两次自动重试、取消/晚到结果拦截、阅读页与显式 context opt-in 全部有落库或运行入口 | `src/ai/dream/`、`DreamReaderScreen.tsx`、`DreamDeckPager.tsx` | dream policy/prompt/recovery/repository tests |
...
5. 角色声音使用任务所属会话的 `thread.roleSnapshotJson`，重试期间即使全局角色卡被编辑，也不改变同一冻结任务的角色输入。
...
`完整问答落库 → 幂等角色轮次 → 本地候选/零成本排除 → 场景唯一 seed + 持久 roll → 结构化语义分类 → 原子配额预留 → 冻结 20 完整轮和会话角色快照 → 独立生成/严格解析 → 提交前复核 branch、lineage、消息版本与取消状态 → 事务提交 artifact/成功计数 → 来源消息后替换卡片 → 阅读/显式 context opt-in`

## 4. 验证结果与环境记录

- 聚焦测试：80/80 通过，覆盖日记、梦境、时间线锚点和启动图策略。
- 完整测试：978 项，963 通过、15 跳过、0 失败；TypeScript `pnpm typecheck` 与 `git diff --check` 通过。
