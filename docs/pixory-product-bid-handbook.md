...
用户可以设置隐私空间密码，在普通空间之外维护独立的 personal 数据库和文件目录。隐私空间支持解锁、锁定、后台超时锁定、截屏防护、独立备份与重置。
...
| 隐私空间 | 独立数据库、独立文件目录、密码、锁定、截屏防护 | 已实现 |
...
当前隐私空间提供基础本地隔离、密码校验、锁定和截屏保护。除加密导出包外，不应将其表述为“端到端加密本地保险箱”。
...
普通空间与隐私空间在数据库和文件目录上分离，并提供密码、后台锁定、截屏防护、任务 token 失效等机制，适合区分普通素材和个人敏感素材。
...
| 隐私保护 | personal 空间锁定、截屏防护和任务失效机制可验证 |
