...
Pixory 支持通过官方网站托管的 JSON 提供轻量远程更新和公告，更新弹窗默认跳转官网获取区；官网主下载使用服务器直下最新版 APK，GitHub Release 保留备用和历史版本。
...
| 日期 | 版本 | 更新摘要 |
| --- | --- | --- |
| 2026-07-30 | 2.7.4 | 发布陪伴内心运行时与角色日记增强，完善长聊天恢复、答案级引用、受管备份与 Personal 隔离，并更新 Android 图标和启动屏 |
| 2026-08-03 | 2.7.5 | 支持选择文本编辑后自动进入内联气泡修改，优化文本不可选中防止长按冲突，修复输入框动画闪烁伪影 |
| 2026-07-27 | 2.6.10 | 发布可重建的 Memory v1：三层 Claim 账本、低噪声检索、可控看板、原生 v2 导入导出以及 Personal/思考内容隔离；Android release 仅包含真机 ARM ABI |
| 2026-06-24 | 2.5.2 | 新增 AI 连续性导入、导入记忆审读门禁、聊天附件可回放，并修复模型删除清理与视频播放器随机逻辑 |
| 2026-06-24 | 2.5.2 OTA | 补齐外部 txt/md 解析不足时的模型辅助结构恢复、导入评审显式 fan-out、会话内直达角色卡导入与小型回退提示，以及模型长按多选批量清理 |
| 2026-06-20 | 2.4.10 | 优化 AI 长聊天流式性能、提示词缓存稳定性和 SillyTavern 兼容角色包导出 |
| 2026-06-13 | 2.4.5 | 优化 AI 工作台最近角色与最近聊天布局，过滤空白新聊天历史，并将应用内更新默认切到官网获取区 |
