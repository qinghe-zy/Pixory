...
[访问官网](https://mist01.com/) · [下载最新版 APK](https://github.com/qinghe-zy/Pixory/releases/latest) · 当前版本 `2.1.17`
...
这一版继续优化 AI 长聊天和本地记忆控制：

- 长期记忆支持安全校准，可以新增、更新、保留或标记过期。
- 旧偏好被新偏好替代时，过期记忆不会继续进入后续回复。
- 聊天中会低调显示记忆反馈，支持撤销、编辑和进入记忆管理。
- 记忆管理页可筛选当前记忆和已过期记忆，过期原因更清楚。
- 手动添加的记忆保持用户最高权限，不会被模型自动覆盖或删除。
