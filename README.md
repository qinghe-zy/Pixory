...
[访问官网](https://mist01.com/#download) · [服务器直下](https://mist01.com/downloads/Pixory-v2.7.4.apk) · [GitHub 备用](https://github.com/qinghe-zy/Pixory/releases/latest) · 当前版本 `2.7.4`
...
- 陪伴内心运行时与角色日记增强：情绪、关系、梦境、离线思绪和日记会在受控边界内影响角色表达；Android 后台唤醒与前台协调让任务在中断后可恢复。
- 长聊天体验继续优化：回到底部、消息上下文操作、生成恢复与直接发言更稳定，答案级引用更容易追溯资料来源。
- 强化受管备份、Personal 恢复生命周期和上下文覆盖；敏感空间、思考内容和材料边界继续保持隔离。
- 更新 Android 桌面图标与 Android 12+ 启动屏，发布包仍仅包含 ARM 真机 ABI。
...
- 服务器直下（最新版 APK）：[https://mist01.com/downloads/Pixory-v2.7.4.apk](https://mist01.com/downloads/Pixory-v2.7.4.apk)
...
维护发布信息时，需要保持版本号、版本码、服务器 APK 文件名、Release 页面和远程 JSON 一致。先运行 `pnpm release:android`，它会 clean、仅构建真机 ARM ABI 并拒绝含模拟器 ABI 的产物；部署官网时可用 `scripts/deploy-docs-mist01.ps1 -ApkPath output/release/Pixory-v2.7.4.apk -Version 2.7.4` 上传最新版 APK，并清理服务器旧 APK。
