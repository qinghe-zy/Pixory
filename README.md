...
[访问官网](https://mist01.com/) · [下载最新版 APK](https://github.com/qinghe-zy/Pixory/releases/latest) · 当前版本 `2.1.19`
...
这一版继续补强 AI 聊天分支、历史抽屉和角色页体验：

- 编辑原消息和重新生成会保留旧路径，并按当前分支隔离提示词、记忆和搜索上下文。
- 综合记录抽屉最近项支持长按重命名和删除，命中当前会话时会同步标题或切到新聊天。
- 思考过程折叠时不再流式占位，展开时正常渲染，避免内容被裁切。
- 角色页头像候选图默认收起，选择 IP 后再展开，首屏更紧凑。
- 聊天输入框边框更清晰，新聊天与返回路径更稳定。
