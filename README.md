...
这一版重做长期记忆与陪伴运行时的底层链路，重点是让聊天更连贯、记忆更可控，同时避免普通聊天多花远程模型调用：
...
- 陪伴内心运行时 V1 已接入：关系、情感、时间与事件状态仅作为内部上下文影响角色表达，不向用户展示分数；角色日记、离线思绪与梦境共享受控的 artifact 与话题仲裁边界。
- 官方 DeepSeek V4 请求会读取原生缓存/用量字段并保持稳定提示词前缀；不替用户干预其他模型选择。
