...
[访问官网](https://mist01.com/) · [下载最新版 APK](https://github.com/qinghe-zy/Pixory/releases/latest) · 当前版本 `2.1.16`
...
这一版重点优化 AI 长聊天体验和本地记忆能力：

- 会话设置新增“深度记忆”开关，默认关闭，开启后本地保存摘要和可复用记忆。
- 普通聊天和深度记忆模式都使用最近 20 条短期上下文。
- 长聊天列表改为虚拟化加载，默认显示最近消息并可加载更早记录。
- 流式回复改为增量更新并节流写库，减少聊天记录变多后的卡顿。
- 深度记忆只作为背景参考，不覆盖当前最新要求、角色指令或资料事实。
