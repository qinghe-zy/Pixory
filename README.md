...
[访问官网](https://mist01.com/#download) · [服务器直下](https://mist01.com/downloads/Pixory-v2.7.7.apk) · [GitHub 备用](https://github.com/qinghe-zy/Pixory/releases/latest) · 当前版本 `2.7.7`
...
这一版把长聊天路线、角色日记/梦境和 Android 启动体验进一步收口，重点是让恢复路径可依赖、卡片位置稳定、低消息量仍能生成，并让启动图在 Android 12 上保持完整构图：

- 聊天进入、刷新、搜索、历史回看和创作路线树统一使用已持久化的分支路线，避免进入错误线程或显示旧的最新消息。
- 日记与梦境卡片固定在触发消息之后；消息不足时回退到最近 30 个完整对话轮，并为模型上下文附带北京时间时间戳。
- 梦境每日最多自动生成两次，失败不消耗冷却，重试沿用冻结输入并可恢复；日记支持手动和自动触发。
- Android 12 启动图改用缩小的透明前景并保留图片、视频、星星等外围装饰，使用 `contain` 避免圆形安全区切掉主体；发布包仍仅包含 ARM 真机 ABI。
...
- 服务器直下（最新版 APK）：[https://mist01.com/downloads/Pixory-v2.7.7.apk](https://mist01.com/downloads/Pixory-v2.7.7.apk)
...
维护发布信息时，需要保持版本号、版本码、服务器 APK 文件名、Release 页面和远程 JSON 一致。先运行 `pnpm release:android`，它会 clean、仅构建真机 ARM ABI 并拒绝含模拟器 ABI 的产物；部署官网时可用 `scripts/deploy-docs-mist01.ps1 -ApkPath output/release/Pixory-v2.7.7.apk -Version 2.7.7` 上传最新版 APK，并清理服务器旧 APK。
