...
[访问官网](https://mist01.com/) · [下载最新版 APK](https://github.com/qinghe-zy/Pixory/releases/latest) · 当前版本 `2.1.18`
...
这一版继续补强 AI 聊天与角色卡导入的细节安全：

- 角色卡“编辑后保存”会保留开场白、候选开场白、标签、来源类型和原始 JSON。
- 载入已保存角色前会保护当前未保存草稿，避免误触覆盖。
- 编辑已有角色卡后会更新原卡，不再产生同名重复卡片。
- 新聊天和打开其他聊天线程时，输入框会以 0.5 秒浮出动画提示用户。
- 输入框动画只响应明确的聊天切换，不会被键盘、流式生成或布局变化抢占。
