...
[访问官网](https://mist01.com/#download) · [服务器直下](https://mist01.com/downloads/Pixory-v2.8.5.apk) · [GitHub 备用](https://github.com/qinghe-zy/Pixory/releases/latest) · 当前版本 `2.8.5`
...
这一版本重点重构了隐私空间的密码与安全体系，增强了视觉设备自适应，并优化了多项聊天交互与素材体验：

- **隐私安全重构**：全新设计的“密码与安全”设置页面，分类更清晰成熟；新增 6 字符（XXX-XXX）易读自定义恢复密钥机制与展示卡片，并修复了恢复密钥防爆破锁定及密码类型重置错位的安全隐患。
- **宽屏视觉自适应**：所有页面背景图统一采用等比缩放铺满（cover 模式），全面适配平板与大屏设备。
- **聊天与梦境管理升级**：发送图文消息时图片在 AI 开始回复时立即呈现，无需等待生成结束；聊天页面支持长按彻底删除“生成失败”或卡住的梦境卡片；支持将临时会话的提示词和头像一键保存为新角色卡。
- **交互与稳定性细节**：延长素材列表缩略图长按响应时间以防滚动误触；修复新机安装时数据库升级脚本重复字段引发的崩溃；优化 DeepSeek 官方识图模型同步白名单。

### Pixory 2.8.2

这一版本重点引入了更具沉浸感的交互视觉与全屏书写体验优化，为原本的页面注入了生命力与动态氛围：
...
- 服务器直下（最新版 APK）：[https://mist01.com/downloads/Pixory-v2.8.5.apk](https://mist01.com/downloads/Pixory-v2.8.5.apk)
...
Pixory 的 Manifest V2 备份覆盖数据库、原图/视频、缩略图、AI 文档、聊天附件和角色头像；AI 线程、消息、记忆、角色卡和材料索引随数据库进入备份。导入会校验路径、大小与 SHA-256，并重写受管文件 URI、避免重复项，再以事务方式合并恢复。

仍有明确安全边界：SecureStore 中的 API Key 不会导出；换机或重装时仍应保管原始备份包与密码，个人空间包只能在通过密码验证后恢复。自动化测试已覆盖 Manifest、完整性校验、事务合并和 URI 重写；真实 Android 设备的“导出 → 清空/重装 → 导入 → 打开文件”全链路验收仍应在发布前完成。
...
| Prompt/缓存 | fast-path、分层 retrieval、真实上下文窗口、stable prefix/cache key 保护、官方 DeepSeek V4 原生缓存/用量观测 |
| 陪伴内心运行时 | 内部事件、关系、情感、时间与姿态投影；角色日记、离线思绪、梦境及单槽话题仲裁，分数不前台展示 |
...
| 备份导入 | Manifest V2：数据库、原图/视频、缩略图、AI 文档、聊天附件、角色头像、IP 备份、加密包与资源包导入；路径/大小/SHA-256 校验和 URI 重写 |
...
维护发布信息时，需要保持版本号、版本码、服务器 APK 文件名、Release 页面和远程 JSON 一致。先运行 `pnpm release:android`，它会 clean、仅构建真机 ARM ABI 并拒绝含模拟器 ABI 的产物；部署官网时可用 `scripts/deploy-docs-mist01.ps1 -ApkPath output/release/Pixory-v2.8.5.apk -Version 2.8.5` 上传最新版 APK，并清理服务器旧 APK。
