...
4. Update all release version sources together:
   ...
   - `docs/updates.html`
   - `docs/sitemap.xml`
   - `docs/manual.html` (官网阅读文档下拉卡片的版本号)
...
- 打包可在 `local-work`（推荐，保留完整版本上下文）或 `main` 执行 `pnpm release:android`。`scripts/build-android-release.ps1` 负责 clean/build 和文档预检，`scripts/version-document-workflow.ps1` 负责 APK 成功后的归档与下一版本文档初始化，`scripts/release-handoff.ps1` 使用临时 Git index 从 `main` 生成只含公开路径的 release commit，再创建 tag、推送 GitHub 并上传 GitHub Release 说明/APK；不会推送 `local-work`。
...
- To publish local development, run the packaging workflow from `local-work` or `main`; the handoff creates/updates only the public `main` ref. Never merge the full `local-work` branch into `main`, and never cherry-pick a commit that contains private paths.
- Before pushing, verify `git branch --show-current`, `git branch -vv`, and the public tree generated by the handoff. Push routine work only with `git push origin main`; do not use `git push --all`, `git push --mirror`, `--no-verify`, or force-push for routine work.
- The repository pre-push hook rejects `local-work`, `local/*`, non-`main` branch pushes, non-`origin` remotes, and accidental private branch publication. It permits a private-branch push only during `scripts/release-handoff.ps1` with the packaging-only `PIXORY_RELEASE_HANDOFF=1` token; do not bypass it manually.
- Release tags are created on the generated public `main` commit and pushed only by the completed release workflow. Do not push release commits or tags to `gitee`.
