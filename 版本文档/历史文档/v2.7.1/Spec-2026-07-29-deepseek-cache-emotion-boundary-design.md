...
状态：阶段 1 已实施并合入 `main`；本文件保留缓存与情感上下文的设计边界，以及尚未进入实施的后续阶段。

## 1. 文档目的

本文件保存 Pixory 针对 DeepSeek 流式聊天、Provider 前缀缓存、记忆三车道和情感运行时的已确认优化方向，避免后续代码合并或会话切换后丢失上下文。

实施快照（2026-07-29）：官方 DeepSeek V4 模型兼容、原生缓存字段与用量观测已由 `6eab018 Optimize official DeepSeek caching compatibility` 实施；陪伴运行时 V1 已由 `a51229c Merge companion inner life runtime into main` 合入。完整的历史包装信封持久化、动态段冗余收敛和后续缓存阶段仍未自动视为完成。任何继续实施都必须以最新 `main` 的源码、数据库 schema、测试和真实 DeepSeek 用量字段为准重新核对接入点。
