...
export function buildMemoryReconciliationPrompt(input: {
  ...
  return [
    ...
    'JSON 结构：{"summary":"会话摘要","decisions":"已确认事项","openQuestions":"待跟进问题","memories":[{"scope":"ip或thread","type":"preference|fact|decision|instruction|task|correction","content":"新增内容(需为独立原子事实，包含具体主语和全名)","confidence":0.1到1,"importance":1到5}],"operations":[{"op":"add|update|stale|keep","targetMemoryId":"候选旧记忆id，可选","scope":"ip或thread","type":"preference|fact|decision|instruction|task|correction","content":"新增或更新后的内容(需为独立原子事实，包含具体主语和全名)","confidence":0.1到1,"importance":1到5,"reason":"简短原因"}]}',
    'scope 只允许 ip 或 thread；跨会话稳定偏好（针对当前项目的属性）用 ip，本会话局部决策和任务用 thread。严格禁止写入 global 记忆。',
    '候选旧记忆：',
    oldMemories,
    '最近对话：',
