...
export function DreamChatCard({actionLabel,createdAt,failureMessage,title,onOpen,status,onRetry}:{actionLabel?:string;createdAt:string;failureMessage?:string;title:string;onOpen?:()=>void;status?:'generating'|'failed'|'waiting_model'|'completed';onRetry?:()=>void}){
  const failed = status === 'failed' || status === 'waiting_model';
  return (
    <View style={styles.host}>
      <Pressable accessibilityLabel={failed?(actionLabel??'重试梦境制作'):'查看梦境'} accessibilityRole="button" onPress={failed?onRetry:onOpen} disabled={status==='generating'} style={({pressed})=>[styles.card,pressed&&styles.pressed]}>
