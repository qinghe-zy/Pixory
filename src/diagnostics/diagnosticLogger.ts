...
export function recordDiagnosticEvent(event: DiagnosticEventInput): void {
  if (enabledBySpace.get(event.space) !== true) return;
...
export function recordDiagnosticWindow(input: { id: string; space: 'normal' | 'personal'; operationId?: string; occurredAtUtc: string; traceId: string; payload: Record<string, unknown> }): void { if (enabledBySpace.get(input.space) !== true) return; windowBufferFor(input.space).record({ traceId: input.traceId, space: input.space, eventType: 'diagnostic_window', occurredAt: input.occurredAtUtc, payload: input as unknown as Record<string, unknown> }); }
export function recordDiagnosticOperation(input: DiagnosticOperationInput): void { if (enabledBySpace.get(input.space) !== true) return; operationBufferFor(input.space).record({ traceId: input.traceId, space: input.space, eventType: 'operation_summary', occurredAt: input.occurredAtUtc, threadIdHash: input.threadIdHash, generationId: input.generationId, payload: input as unknown as Record<string, unknown> }); }
export function recordDiagnosticIncident(input: DiagnosticIncidentInput): void { if (enabledBySpace.get(input.space) !== true) return; incidentBufferFor(input.space).record({ traceId: input.traceId, space: input.space, eventType: 'incident', occurredAt: input.occurredAtUtc, threadIdHash: input.threadIdHash, generationId: input.generationId, payload: input as unknown as Record<string, unknown> }); }
