...
test('AI chat persists and exposes message versions for edits and regenerations', () => {
  ...
  assert.match(schema, /DATABASE_VERSION = (?:5[1-9]|6[0-9])/);
...
test('AI chat uses an inverted list pinned to offset zero without forced scrollToEnd loops', () => {
  ...
  assert.match(latestVisibilityBody, /shouldShowScrollToLatest\(offsetY\)/);
  ...
  assert.match(chat, /<AiScrollToLatestButton\s+bottomOffset=\{composerShellHeight \+ spacing\[3\] \+ spacing\[1\.5\]\}\s+generating=\{generating\}\s+visible=\{showScrollToLatest && !inlineEditingActive\}\s+onPress=\{handleReturnToLatestPress\}/);
...
test('AI chat attachment pipeline is replayable and budget-safe', () => {
  ...
  assert.match(service, /supportsDeepSeekVision/);
  assert.match(service, /assertDeepSeekVisionRequest/);
...
test('AI long chat rendering memoizes message rows and precomputes avatar grouping', () => {
  ...
  assert.match(chat, /const displayMessages = nextVisibleMessages\.map/);
  assert.match(chat, /buildCompanionArtifactTimeline\(\{/);
  assert.match(chat, /timelineItems\.forEach\(\(item\) =>/);
  assert.match(chat, /const nextInvertedMessageItems = nextVisibleMessageItems\.slice\(\)\.reverse\(\)/);
...
test('AI chat polish avoids redundant scroll state updates and clears transient timers', () => {
  ...
  assert.match(chat, /showScrollToLatestRef/);
  assert.match(chat, /const nextShowScrollToLatest = shouldShowScrollToLatest\(contentOffset\.y\)/);
  assert.doesNotMatch(chat, /hasUnseenStreamingUpdate \|\| shouldShowScrollToLatest\(contentOffset\.y\)/);
  ...
  assert.doesNotMatch(latestButton, /BlurView/);
  assert.match(latestButton, /metrics\.scrollToLatestVisualSize/);
  assert.match(latestButton, /metrics\.minTouchSize/);
  assert.match(latestButton, /color=\{aiLightColors\.ink\}/);
...
test('AI history archive restore swipe clips the action background to the row', () => {
  ...
  assert.match(history, /renderItem=\{\(\{ item: thread, index \}\) => \{[\s\S]*<View>\s*\{groupLabel !== previousGroupLabel[\s\S]*<View style=\{styles\.swipeWrap\}>/);
