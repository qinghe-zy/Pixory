...
test('website release-facing files reference the current 2.1.8 release', () => {
  assert.match(read('docs/download.html'), /<div class="version-number">2\.1\.8<\/div>/);
  assert.match(read('docs/updates.html'), /<h2 class="section-title reveal">2\.1\.8<\/h2>/);
  assert.match(read('README.md'), /当前版本 `2\.1\.8`/);
  assert.match(read('docs/pixory-product-bid-handbook.md'), /适用版本：Pixory 2\.1\.8/);
