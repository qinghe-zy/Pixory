...
test('website release-facing files reference the current 2.7.6 release', () => {
  assert.match(read('docs/index.html'), /当前版本：2\.7\.6/);
  assert.match(read('docs/m.html'), /当前版本：2\.7\.6/);
  assert.match(read('app.json'), /"version": "2\.7\.6"/);
  assert.match(read('README.md'), /当前版本 `2\.7\.6`/);
  assert.match(read('docs/pixory-product-bid-handbook.md'), /适用版本：Pixory 2\.7\.6/);
  assert.match(read('README.md'), /https:\/\/mist01\.com\/downloads\/Pixory-v2\.7\.6\.apk/);
  assert.match(read('docs/index.html'), /https:\/\/mist01\.com\/downloads\/Pixory-v2\.7\.6\.apk/);
  assert.match(read('docs/m.html'), /https:\/\/mist01\.com\/downloads\/Pixory-v2\.7\.6\.apk/);
  assert.match(read('package.json'), /"version": "2\.7\.6"/);
...
test('website sitemap lastmod is synchronized with the release update date', () => {
  ...
  const matches = sitemap.match(/<lastmod>2026-08-03<\/lastmod>/g) ?? [];
