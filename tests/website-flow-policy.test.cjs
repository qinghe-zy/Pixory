...
test('website release-facing files reference the current 2.6.5 release', () => {
  assert.match(read('docs/index.html'), /当前版本：2\.6\.5/);
  assert.match(read('docs/update-version.json'), /"version": "2.6.5"/);
  assert.match(read('docs/index.html'), /SQLite/);
  assert.match(read('README.md'), /当前版本 `2\.6\.5`/);
  assert.match(read('docs/pixory-product-bid-handbook.md'), /适用版本：Pixory 2\.6\.5/);
  assert.match(read('README.md'), /https:\/\/mist01\.com\/downloads\/Pixory-v2\.6\.5\.apk/);
  assert.match(read('docs/index.html'), /https:\/\/mist01\.com\/downloads\/Pixory-v2\.6\.5\.apk/);
...
test('website sitemap lastmod is synchronized with the release update date', () => {
  ...
  const matches = sitemap.match(/<lastmod>2026-07-12<\/lastmod>/g) ?? [];
