...
test('website release-facing files reference the current 2.8.4 release', () => {
  assert.match(read('docs/index.html'), /当前版本：2\.8\.4/);
  assert.match(read('docs/m.html'), /当前版本：2\.8\.4/);
  assert.match(read('app.json'), /"version": "2\.8\.4"/);
  assert.match(read('README.md'), /当前版本 `2\.8\.4`/);
  assert.match(read('docs/pixory-product-bid-handbook.md'), /适用版本：Pixory 2\.8\./);
  assert.match(read('README.md'), /https:\/\/mist01\.com\/downloads\/Pixory-v2\.8\.4\.apk/);
  assert.match(read('docs/index.html'), /https:\/\/mist01\.com\/downloads\/Pixory-v2\.8\.4\.apk/);
  assert.match(read('docs/m.html'), /https:\/\/mist01\.com\/downloads\/Pixory-v2\.8\.4\.apk/);
  assert.match(read('package.json'), /"version": "2\.8\.4"/);
...
test('website sitemap lastmod is synchronized with the release update date', () => {
  ...
  const matches = sitemap.match(/<lastmod>2026-09-10<\/lastmod>/g) ?? [];
