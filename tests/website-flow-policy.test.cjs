...
test('website release-facing files reference the current 2.4.0 release', () => {
  assert.match(read('docs/download.html'), /<div class="version-number">2\.4\.0<\/div>/);
  assert.match(read('docs/download.html'), /versionCode 240/);
  assert.match(read('docs/updates.html'), /<h2 class="section-title reveal">2\.4\.0<\/h2>/);
  assert.match(read('docs/updates.html'), /AI 创作路线树/);
  assert.match(read('docs/updates.html'), /多分支折叠/);
  assert.match(read('docs/features.html'), /会话整理/);
  assert.match(read('README.md'), /当前版本 `2\.4\.0`/);
  assert.match(read('docs/pixory-product-bid-handbook.md'), /适用版本：Pixory 2\.4\.0/);
...
test('website sitemap lastmod is synchronized with the release update date', () => {
  ...
  const matches = sitemap.match(/<lastmod>2026-05-29<\/lastmod>/g) ?? [];
