...
test('fresh AI database migration skips branch columns already created by the base AI schema', () => {
  ...
  assert.match(schema, /CREATE TABLE IF NOT EXISTS ai_threads[\s\S]*modelTitleGeneratedAt TEXT/);
  assert.match(schema, /CREATE TABLE IF NOT EXISTS ai_threads[\s\S]*thinkingDisabled INTEGER NOT NULL DEFAULT 0/);
